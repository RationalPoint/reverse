...
[1] Xander Faber and Jon Grantham. "On Integers Whose Sum is the Reverse of
    their Product". In preparation, 2021.

See the notes below for improvements that do not appear in [1]. Some non-core
functionality depends on Sage or pygraphviz.
...
article. The following proposition is useful for eliminating some of the work in
construction RSPAutomata:

Proposition. Suppose that a \le b is a reversed sum-product pair base
\beta. Then gcd( \beta-1, a-1 ) = 1.

Proof.  Suppose the expansion of a+b in base \beta is

   a + b = n_r \beta^r + n_{r-1} \beta^{r-1} + \cdots + n_1 \beta + n_0.

Then the expansion of ab must be

   ab = n_0 \beta^r + n_1 \beta^{r-1} + \cdots + n_{r-1} \beta + n_r.

Reducing modulo \beta - 1 gives 

  a + b  \equiv n_r + n_{r-1} + \cdots + n_1 + n_0 \equiv ab \pmod{\beta-1}.

Massaging this congruence, we find that 

  (a-1)(b-1) \equiv 1 \pmod{\beta-1}.  

We conclude that a-1 and \beta - 1 are coprime. QED
...
class RSPAutomaton():
  ...
  def odd_state(self):
    r"""
    Return the odd state, if it exists
    """
    qodd = self._odd
    if qodd is not None:
      return qodd
  ...
  def out_states(self, q):
    r"""
    Return the list of outward states from q
    """
    outs = self._outs
    if outs is not None:
      return outs[q]
